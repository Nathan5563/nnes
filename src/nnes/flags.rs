...
impl NNES
{
    ...
    pub fn set_flag(&mut self, flag: Flag, status: bool)
    {
        ...
        {
            Flag::Carry => { 
                if status { self.flags |= CF; }
                else { self.flags &= NEG_CF; }
            }
            Flag::Zero => { 
                if status { self.flags |= ZF; }
                else { self.flags &= NEG_ZF; }
            }
            Flag::InterruptDisable => { 
                if status { self.flags |= IF; }
                else { self.flags &= NEG_IF; }
            }
            Flag::DecimalMode => { 
                if status { self.flags |= DF; }
                else { self.flags &= NEG_DF; }
            }
            Flag::Break => { 
                if status { self.flags |= BF; }
                else { self.flags &= NEG_BF; }
            }
            Flag::Overflow => { 
                if status { self.flags |= VF; }
                else { self.flags &= NEG_VF; }
            }
            Flag::Negative => { 
                if status { self.flags |= NF; }
                else { self.flags &= NEG_NF; }
            }
        }
    }
